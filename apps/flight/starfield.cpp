...
static int RandomInt(int max_value) {
  unsigned int random_value = rand() % max_value;
  return random_value;
}
...
void Starfield::ResetAll() {
  ...
  for (uint8_t i = 0; i < STARFIELD_COUNT; i++) {
    _stars[i].Reset(RandomInt(2 * STARFIELD_MARGIN + frame_buffer->bounds.w) -
                        STARFIELD_MARGIN,
                    RandomInt(2 * STARFIELD_MARGIN + frame_buffer->bounds.h) -
                        STARFIELD_MARGIN);
  }
}
...
void Star::Reset(int16_t x, int16_t y) {
  position.x = x;
  position.y = y;
  color = RandomInt(STAR_COLOR_LENGTH);
  speed = (((float)RandomInt(75)) / 100) + 0.25f;
}

// Move each star based on ship and star speed.
void Starfield::Scroll(blit::Vec2 ship_velocity) {
  blit::Vec2 vel;
  for (uint8_t i = 0; i < STARFIELD_COUNT; i++) {
    vel = ship_velocity;
    ...
    if (_stars[i].position.x > frame_buffer->bounds.w + STARFIELD_MARGIN) {
      _stars[i].Reset(-STARFIELD_MARGIN, RandomInt(frame_buffer->bounds.h));
    } else if (_stars[i].position.x < -STARFIELD_MARGIN) {
      _stars[i].Reset(frame_buffer->bounds.w + STARFIELD_MARGIN,
                      RandomInt(frame_buffer->bounds.h));
    } else if (_stars[i].position.y >
               frame_buffer->bounds.h + STARFIELD_MARGIN) {
      _stars[i].Reset(RandomInt(frame_buffer->bounds.w), -STARFIELD_MARGIN);
    } else if (_stars[i].position.y < -STARFIELD_MARGIN) {
      _stars[i].Reset(RandomInt(frame_buffer->bounds.w),
                      frame_buffer->bounds.h + STARFIELD_MARGIN);
    }
  }
}

void Starfield::Draw(blit::Vec2 ship_velocity) {
